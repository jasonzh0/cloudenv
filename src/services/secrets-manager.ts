...
export class SecretsManager {
  ...
  constructor(environment: Environment) {
    ...
    // Only initialize GCP client for GCP provider
    if (environment.provider === "gcp") {
      this.client = new SecretManagerServiceClient();
    } else if (environment.provider === "aws") {
      // AWS support is not yet implemented
      throw new Error(
  ...
  private buildFilter(): string {
    ...
    if (this.environment.prefix) {
      filters.push(
        `name:projects/${this.environment.projectId}/secrets/${this.environment.prefix}*`,
      );
    }

    if (this.environment.labels) {
      const labelFilters = Object.entries(this.environment.labels)
